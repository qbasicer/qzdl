...
void ZDLMainWindow::launch(){
	...
	wchar_t* cmd = (char*)malloc((compose.length()+1)*sizeof(wchar_t));
	swprintf(cmd,compose.length()+1,"%s",compose.toStdString().c_str());
	...
	BOOL rc = CreateProcess(exec.toStdWString().c_str(), cmd, NULL, NULL, FALSE, NORMAL_PRIORITY_CLASS, NULL, workingDirectory.toStdString().c_str(), lpStartupInfo, lpProcessInfo);
	free(cmd);
	if(rc == FALSE){
